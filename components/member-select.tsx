...
export const MemberSelect: FC<MemberSelectProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative z-50 flex h-10 w-full cursor-pointer rounded-md border border-input bg-slate-100 px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
      )}
      {...props}
      ...
      >
        {options.map((option) => (
          <li
            key={option.user.id}
            className={cn(
              "flex items-center gap-2 rounded-md bg-slate-200 px-3 py-2 hover:bg-foreground/80 hover:text-white",
